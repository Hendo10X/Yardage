...
export const FeatureSection = () => {
  return (
    <div className='px-6 md:px-14 lg:px-28 py-6 md:py-28'>
        <div>
            <h1 className='flex flex-col font-bold text-[30px] md:text-[50px] leading-[100%]'>
                <span>Your school,</span> 
                <span className='text-primary'>your market.</span>
            </h1>
            <p className='py-8 leading-[100%] text-[14px] md:text-[18px] text-[#828181] flex flex-col gap-2'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-16 py-12 w-full lg:w-[80%]'>
            {features.map((feature, index) => (
                <div key={index} className='flex flex-col gap-2'>
                    ...
                    <h2 className='md:text-[24px] text-[20px] font-medium'>{feature.title}</h2>
                    <p className='md:text-[18px] text-[14px] text-[#828181] w-[80%] lg:w-[60%]'>{feature.description}</p>
                </div>
            ))}
        </div>
    </div>
